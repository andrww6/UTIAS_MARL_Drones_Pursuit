...
class Scenario_o_multi_layout(Scenario_o_base):
    def __init__(self, quads_mode, envs, num_agents, room_dims):
        ...
        self.min_goal_distance = 4.0  # Minimum distance between goals
        self.goal_speed = 2.0  # Increased speed of goal movement
        self.goal_safe_distance = 3.0  # Safe distance between goals
        self.zigzag_distance = 2.0  # Distance to travel before changing direction
                    ...
                    if self.check_goal_collision(self.goal_positions[i], self.goal_positions[j]):
                        ...
                        if distance > 0:
                            ...
                            if overlap > 0:
                                ...
                                # Ensure goals stay within room boundaries after adjustment
                                self.goal_positions[i][:2] = np.clip(self.goal_positions[i][:2],
                                                                   [-room_x/2 + margin, -room_y/2 + margin],
                                                                   [room_x/2 - margin, room_y/2 - margin])
