import torch
import torch.nn as nn


class ActorCriticPOAM(nn.Module):
    def __init__(self, action_dim, embedding_size):
        super(ActorCriticPOAM, self).__init__()

        self.embeding_size = embedding_size
        # actor
        self.layer1 = nn.Linear(2, 128)
        self.layer2 = nn.Linear(2, 128)
        self.layer3 = nn.Linear(2, 128)
        self.layer4 = nn.Linear(128 + 128 + 128 + self.embeding_size, 128)
        self.layer5 = nn.Linear(128, 128)
        self.layer6 = nn.Linear(128, action_dim)
        self.logstd = nn.Parameter(torch.zeros((1, action_dim)))
        # critic
        self.layer7 = nn.Linear(2, 128)
        self.layer8 = nn.Linear(2, 128)
        self.layer9 = nn.Linear(2, 128)
        self.layer10 = nn.Linear(128 + 128 + 128, 128)
        self.layer11 = nn.Linear(128, 128)
        self.layer12 = nn.Linear(128, 1)

        torch.nn.init.orthogonal_(self.layer1.weight)
        torch.nn.init.orthogonal_(self.layer2.weight)
        torch.nn.init.orthogonal_(self.layer3.weight)
        torch.nn.init.orthogonal_(self.layer4.weight)
        torch.nn.init.orthogonal_(self.layer5.weight)
        torch.nn.init.orthogonal_(self.layer6.weight)
        torch.nn.init.orthogonal_(self.layer7.weight)
        torch.nn.init.orthogonal_(self.layer8.weight)
        torch.nn.init.orthogonal_(self.layer9.weight)
        torch.nn.init.orthogonal_(self.layer10.weight)
        torch.nn.init.orthogonal_(self.layer11.weight)
        torch.nn.init.orthogonal_(self.layer12.weight)

        torch.nn.init.constant_(self.layer1.bias, 0.0)
        torch.nn.init.constant_(self.layer2.bias, 0.0)
        torch.nn.init.constant_(self.layer3.bias, 0.0)
        torch.nn.init.constant_(self.layer4.bias, 0.0)
        torch.nn.init.constant_(self.layer5.bias, 0.0)
        torch.nn.init.constant_(self.layer6.bias, 0.0)
        torch.nn.init.constant_(self.layer7.bias, 0.0)
        torch.nn.init.constant_(self.layer8.bias, 0.0)
        torch.nn.init.constant_(self.layer9.bias, 0.0)
        torch.nn.init.constant_(self.layer10.bias, 0.0)
        torch.nn.init.constant_(self.layer11.bias, 0.0)
        torch.nn.init.constant_(self.layer12.bias, 0.0)

        # encoder
        self.e_layer1 = nn.Linear(2, 128)
        self.e_layer2 = nn.Linear(2, 128)
        self.e_layer3 = nn.Linear(2, 128)
        self.e_layer4 = nn.Linear(128 + 128 + 128, self.embeding_size)

        torch.nn.init.orthogonal_(self.e_layer1.weight)
        torch.nn.init.orthogonal_(self.e_layer2.weight)
        torch.nn.init.orthogonal_(self.e_layer3.weight)
        # torch.nn.init.orthogonal_(self.e_layer4.weight)
        torch.nn.init.constant_(self.e_layer1.bias, 0.0)
        torch.nn.init.constant_(self.e_layer2.bias, 0.0)
        torch.nn.init.constant_(self.e_layer3.bias, 0.0)
        # torch.nn.init.constant_(self.e_layer4.bias, 0.0)

    def forward(self, x, x_global, last_state, num_agent, num_evader=None):
        state_target = x[:, :2 * num_evader]
        state_obs = x[:, 2 * num_evader:2 * num_evader + 2]
        state_ij = x[:, 2 * num_evader + 2:]

        # encoder
        last_state_target = last_state[:, :2 * num_evader]
        last_state_obs = last_state[:, 2 * num_evader:2 * num_evader + 2]
        last_state_ij = last_state[:, 2 * num_evader + 2:]

        last_ij_ori = last_state_ij.reshape(-1, 2)
        last_ij_ori = torch.tanh(self.e_layer1(last_ij_ori))

        ij_num = num_agent - 1
        ij_weight = 1 / ij_num
        last_ij = last_ij_ori[0::ij_num, :] * ij_weight
        for i in range(1, ij_num):
            last_ij += last_ij_ori[i::ij_num, :] * ij_weight

        last_target = last_state_target.reshape(-1, 2)
        last_target_ori = torch.tanh(self.e_layer2(last_target))

        target_wight = 1 / num_evader
        last_target = last_target_ori[0::num_evader, :] * target_wight
        for i in range(1, num_evader):
            last_target += last_target_ori[i::num_evader, :] * target_wight

        last_obs = torch.tanh(self.e_layer3(last_state_obs))

        embedding = torch.hstack((last_target, last_obs, last_ij))
        embedding = torch.tanh(self.e_layer4(embedding))
        # encoder = torch.tanh(self.e_layer5(encoder))
        # embedding = torch.tanh(self.e_layer6(encoder))
        # if random.randint(1, 100000) == 2:
        #     print(f"embeddings: {embedding}")

        # print("+"*10)
        # print(state_target)
        # print(state_obs)
        # print(state_ij)
        # actor
        x_ij_ori = state_ij.reshape(-1, 2)
        x_ij_ori = torch.tanh(self.layer1(x_ij_ori))

        ij_num = num_agent - 1
        ij_weight = 1 / ij_num
        x_ij = x_ij_ori[0::ij_num, :] * ij_weight
        for i in range(1, ij_num):
            x_ij += x_ij_ori[i::ij_num, :] * ij_weight

        x_target = state_target.reshape(-1, 2)
        x_target_ori = torch.tanh(self.layer2(x_target))

        target_wight = 1 / num_evader
        x_target = x_target_ori[0::num_evader, :] * target_wight
        for i in range(1, num_evader):
            x_target += x_target_ori[i::num_evader, :] * target_wight

        # x_target = x_target[0::2, :] * 0.5 + x_target[1::2, :] * 0.5

        x_obs = torch.tanh(self.layer3(state_obs))

        input = torch.hstack((x_target, x_obs, x_ij, embedding))

        actor = torch.tanh(self.layer4(input))
        actor = torch.tanh(self.layer5(actor))
        mean = torch.tanh(self.layer6(actor))
        logstd = self.logstd.expand_as(mean)

        if x_global is not None:
            state_target_global = x_global[:, :2 * num_evader]
            state_obs_global = x_global[:, 2 * num_evader:2 * num_evader + 2]
            state_ij_global = x_global[:, 2 * num_evader + 2:]
            # if  (state_obs_global != state_obs).any() or (state_target_global != state_target).any() or (state_ij_global != state_ij).any():
            #     print(state_obs)
            #     print(state_obs_global)
            #     print("-"*10)
            #     print(state_target)
            #     print(state_target_global)
            #     print("-"*10)
            #     print(state_ij)
            #     print(state_ij_global)
            #     print("-"*10)
            #     1/0
            # critic
            x_ij_ori = state_ij_global.reshape(-1, 2)
            # print(x_ij_ori.shape)
            x_ij_ori = torch.tanh(self.layer7(x_ij_ori))
            # print(state_ij_global.shape)
            # print(x_ij_ori.shape)
            x_ij = x_ij_ori[0::ij_num, :] * ij_weight
            for i in range(1, ij_num):
                x_ij += x_ij_ori[i::ij_num, :] * ij_weight

            x_target = state_target_global.reshape(-1, 2)
            x_target_ori = torch.tanh(self.layer8(x_target))

            target_wight = 1 / num_evader
            x_target = x_target_ori[0::num_evader, :] * target_wight
            for i in range(1, num_evader):
                x_target += x_target_ori[i::num_evader, :] * target_wight

            # x_target = x_target[0::2, :] * 0.5 + x_target[1::2, :] * 0.5

            x_obs = torch.tanh(self.layer9(state_obs_global))

            input = torch.hstack((x_target, x_obs, x_ij))
            critic = torch.tanh(self.layer10(input))
            critic = torch.tanh(self.layer11(critic))
            state_value = self.layer12(critic)
            return mean, logstd, state_value
        else:
            return mean, logstd, None