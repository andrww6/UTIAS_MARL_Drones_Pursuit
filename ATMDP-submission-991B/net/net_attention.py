...
class ZscAttention(nn.Module):
    ...
    def __init__(self, num_action):
        super(ZscAttention, self).__init__()
        ...
        self.attention = nn.Linear(64 + 2 + 2 + 1, 1)
    ...
    def forward(self, x):
        ...
        attention_input = torch.hstack((key,
                                        torch.reshape(torch.hstack((state_loc, state_loc)),
                                                      (-1, state_loc.shape[1]))))
        attention_score = torch.relu(self.attention(attention_input))
