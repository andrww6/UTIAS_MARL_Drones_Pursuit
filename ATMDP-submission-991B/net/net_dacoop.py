...
class ActorCriticDACOOP(nn.Module):
    '''
    ...
    def __init__(self, num_action, num_evader):
        super(ActorCriticDACOOP, self).__init__()
        self.num_evader = num_evader
        self.num_local_state = 2 + 2 * self.num_evader + 1
        self.layer1 = nn.Linear(2, 128)
        self.layer2 = nn.Linear(128 + self.num_local_state, 128)
        self.layer3 = nn.Linear(128, 64)
        self.layer4 = nn.Linear(128, 64)
        self.layer5 = nn.Linear(64, num_action)
        self.layer6 = nn.Linear(64, 1)
        self.key = nn.Linear(128, 64)
        self.attention = nn.Linear(64 + 128 + self.num_local_state, 1)
    ...
    def forward(self, x):
        ...
        '''
        state_loc = x[:, :self.num_local_state]
        state_ij = x[:, self.num_local_state:]
        # embedding
        ...
        if np.isnan(attention_score_normalized.detach().to('cpu').numpy()).any():
            print('attention_score_normalized is nan')
