...
from net.net_poam import ActorCriticPOAM


class AgentPOAM(PursuerAgent):
    def __init__(self, config_args, idx, pretrain_net=None, ctrl_num=1):
        ...
        if pretrain_net is not None:
            ...
            super().__init__(ActorCriticPOAM(config_args["action_dim"], embedding_size=config_args["embedding_size"]), idx, ctrl_num)
        ...
        self.c3 = config_args["c3"]  # encoder and encoder loss bonus coefficient
    ...
    def store_transition(self, state, state_global, action, reward, state_next, state_global_next, last_state, done, memory_index):
        transition = np.hstack(
            (np.ravel(state), np.ravel(state_global), np.ravel(action), np.ravel(reward), np.ravel(state_next),
             np.ravel(state_global_next), np.ravel(last_state), np.ravel(done)))
    ...
    def choose_action(self, state, state_global, last_state, num_agent,  device, if_probs=False):
        # print(state.shape)
        # print(state_global.shape)
        if len(state.shape) == 2 and state.shape[1] == 1:
            # no multi-process
            state = torch.FloatTensor(state.transpose()).to(device)
            last_state = torch.FloatTensor(last_state.transpose()).to(device)
        elif len(state.shape) == 3:
            state = torch.FloatTensor(state).to(device)
            state = torch.squeeze(state, dim=-1)

            state_global = torch.FloatTensor(state_global).to(device)
            state_global = torch.squeeze(state_global, dim=-1)

            last_state = torch.FloatTensor(last_state).to(device)
            last_state = torch.squeeze(last_state, dim=-1)
        mean, logstd, _ = self.eval_net(state, state_global, last_state, num_agent, num_evader=self.num_evader)
    ...
    def learn(self, num_state, action_dim, num_agent, device, training_step):
        ...
        # TODO: 这里的要重新取，中间插了全局state，and state next
        state = torch.FloatTensor(memory[:, :num_state]).to(device)
        state_global = torch.FloatTensor(memory[:, num_state:num_state + num_state*1]).to(device)

        action = torch.FloatTensor(memory[:, num_state + num_state*1 :num_state + num_state*1 + action_dim]).to(device)
        reward = memory[:, num_state + num_state*1 + action_dim:num_state + num_state*1 + action_dim + 1]
        state_next = torch.FloatTensor(
            memory[:, num_state + num_state*1 + action_dim + 1:num_state + num_state*1 + action_dim + 1 + num_state]).to(device)
        state_next_global = torch.FloatTensor(
            memory[:, num_state + num_state*1 + action_dim + 1 + num_state :num_state + num_state*1 + action_dim + 1 + num_state + num_state*1]).to(device)
        last_state = torch.FloatTensor(memory[:, num_state + num_state*1 + action_dim + 1 + num_state + num_state*1: num_state + num_state*1 + action_dim + 1 + num_state + num_state*1 + num_state]).to(device)
        done = memory[:, num_state + num_state*1 + action_dim + 1 + num_state + num_state*1 + num_state: num_state + num_state*1 + action_dim + 1 + num_state + num_state*1 + num_state + 1]

        ######################
        advantage, ref_value = self.calculate_advantage(state,state_global, state_next,state_next_global, last_state, num_agent, reward, done, device)
        ...
        for _ in range(int(self.ppo_epoch * memory.shape[0] / self.minibatch_size)):
            ...
            b_state = state[index, :]
            b_state_global = state_global[index, :]
            ...
            b_last_state = last_state[index, :].detach()
            #### critic loss ####
            # print(b_state.shape)
            # print(b_state_global.shape)
            b_mean, b_logstd, b_state_value = self.eval_net(b_state, b_state_global, b_last_state, num_agent, num_evader=self.num_evader)
            # print(b_state_value.shape)
            # print(b_ref_value.shape)
    ...
    def calculate_advantage(self, state,state_global, state_next,state_next_global, last_state, num_agent, reward, done, device):
        _, _, state_value = self.eval_net(state, state_global, last_state, num_agent, num_evader=self.num_evader)
        _, _, state_value_next = self.eval_net(state_next, state_next_global, last_state, num_agent, num_evader=self.num_evader)
        ################
