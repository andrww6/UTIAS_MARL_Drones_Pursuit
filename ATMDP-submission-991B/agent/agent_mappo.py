...
class AgentMAPPO(PursuerAgent):
    def __init__(self, config_args, idx, pretrain_net=None, ctrl_num=1):
        ...
        if pretrain_net is not None:
            ...
            super().__init__(ActorCriticMAPPO(config_args["action_dim"]), idx, ctrl_num)
    ...
    def store_transition(self, state, state_global, action, reward, state_next,state_global_next, done, memory_index):
        transition = np.hstack(
            (np.ravel(state),np.ravel(state_global), np.ravel(action), np.ravel(reward), np.ravel(state_next),np.ravel(state_global_next), np.ravel(done)))
        if memory_index == 0:
    ...
    def choose_action(self, state, state_global, num_agent,  device, if_probs=False):
        # print(state.shape)
        ...
        mean, logstd, _ = self.eval_net(state, state_global, num_agent, num_evader=self.num_evader)
    ...
    def learn(self, num_state, action_dim, num_agent, device, training_step):
        ...
        done = memory[:, num_state + num_state*1 + action_dim + 1 + num_state + num_state*1: num_state + num_state*1 + action_dim + 1 + num_state + num_state*1 + 1]

        ######################
        advantage, ref_value = self.calculate_advantage(state,state_global, state_next,state_next_global, num_agent, reward, done, device)
        ...
        for _ in range(int(self.ppo_epoch * memory.shape[0] / self.minibatch_size)):
            ...
            b_mean, b_logstd, b_state_value = self.eval_net(b_state, b_state_global, num_agent, num_evader=self.num_evader)
    ...
    def calculate_advantage(self, state,state_global, state_next,state_next_global, num_agent, reward, done, device):
        _, _, state_value = self.eval_net(state, state_global, num_agent, num_evader=self.num_evader)
        _, _, state_value_next = self.eval_net(state_next, state_next_global, num_agent, num_evader=self.num_evader)
        ################
