...
class AgentDQN(PursuerAgent):
    ...
    def __init__(self, config_args, idx, pretrain_net=None, ctrl_num=1):
        if pretrain_net is not None:
            ...
            super().__init__(ActorCriticDQN(config_args["num_action"]), idx, ctrl_num)
            # Hyper Parameters
            ...
            self.target_replace_iter = config_args[
                "target_replace_iter"]  # update frequency of target network

            self.target_net = ActorCriticDQN(config_args["num_action"])  # target network
    ...
    def learn(self, i_episode, num_state, device):
        ...
        q_next_target_net = self.target_net(b_state_next)
        # q_next_eval_net = self.eval_net(b_state_next)
